...
import type { Project, Attachment, StageStatus, Intervention } from '@/lib/types';
import { firestore } from "firebase-admin";
import { users } from '@/data/mock-data'; // Note: users might need to be sourced differently in a real app
...
function processProject(projectDoc: firestore.DocumentSnapshot): Project {
    ...
    const projectData = {
        id: projectDoc.id,
        ...serializeData(data),
    } as unknown as Project; // Cast needed due to serialization
    ...
    // Initialize client-side calculated fields - these will be calculated on client
...
export async function getAllProjects(db: firestore.Firestore): Promise<Project[]> {
    const snapshot = await db.collection('projects').orderBy('projectName').get();
    if (snapshot.empty) {
        return [];
    }
...
export async function addProjectData(db: firestore.Firestore, project: Partial<Omit<Project, 'id'>>) {
    await db.collection('projects').add(project);
};
 
export async function updateProjectData(db: firestore.Firestore, id: string, updates: Partial<Omit<Project, 'id'>>) {
    try {
...
export async function addFileToStage(db: firestore.Firestore, projectId: string, stageId: string, file: Pick<Attachment, 'name' | 'url'>): Promise<boolean> {
    ...
    const { id, ...projectToUpdate } = project;
